...
const ENDPOINT = 'https://wider-long-sky.solana-devnet.quiknode.pro/eadc0cc5edf17a08c7ff6edf722fc7441f44e1ca/'; // 👈 replace with your endpoint
...
async function main() {
    ...
    const signer = await createKeyPairSignerFromBytes(new Uint8Array(bs58.decode(process.env.SIGNER_PRIVATE_KEY)));
    ...
    try {
        bundleId = await lilJitRpc
            .sendBundle(base64EncodedTransactions)
            .send();
